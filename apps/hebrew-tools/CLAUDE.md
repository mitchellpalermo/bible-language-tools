...
- **Merge rule per card is "more progress wins":** higher `repetition` wins, ties broken by later `dueDate`.
- **Stats merge by recency, not by magnitude.** Lifetime counters (`totalReviewed`, `totalCorrect`) take the max, but each date-anchored *pair* moves together with the later date: `streak` follows `lastStreakDate`, and `cardsStudiedToday` follows `lastStudyDate`. Ties break on the larger counter, which keeps the merge symmetric. Picking the larger number instead is the bug this replaced — a device holding streak 12 anchored a month ago beat a live streak of 3, and `loadStats()` then zeroed the result outright because the anchor was neither today nor yesterday, so a stale device destroyed a live streak. The same failure one field over: "most lifetime reviews" is not "studied today", and it overwrote a current `lastStudyDate` with an old one.
