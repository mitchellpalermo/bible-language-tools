...
describe('POST /api/auth/signup', () => {
  ...
  it('redirects to the welcome interstitial on successful signup', async () => {
    ...
    expect(response.headers.get('Location')).toBe('/account/welcome');
  });

  it('sets the gt-auth hint cookie on successful signup', async () => {
    mockAuth.api.signUpEmail.mockResolvedValue(new Response(null, { status: 200 }));
